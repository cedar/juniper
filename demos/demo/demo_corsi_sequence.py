...
from demo.demo1 import toFC, eyeRP, eyeToFC, handRP, handToFC, imgToFC
from demo.demo_corsi import demo_corsi_query


class demo_corsi_sequence(demo_corsi_query):
    def __init__(self, architecture, eye_field, hand_field):
        super().__init__(architecture, eye_field, hand_field, use_query_inputs=False)
    ...
    def tick(self, t):
        ...
        # short break before reproduction signal
        ...
        if t >= 20 + t_present * self.num_objects + t_pause * self.num_objects:
            [img.paste(self.light_off, self.light_positions[i], self.light_off) for i in range(self.num_objects)]
            ImageDraw.Draw(background_img).text((360, 8), "Go", font=self.font, fill=(255, 255, 255))
        ...
        recording_list += [f'mem_ord{i}.activation' for i in range(4)]
        recording_list += ['u.activation', 'v.activation']
        recording_list += ['mem_int.activation', 'mem_cos.activation', 'repr_int.activation', 'repr_cos.activation']
        recording_list += [f'mem_mem{i}.activation' for i in range(4)]
        recording_list += [f'readout{i}.activation' for i in range(4)]
        recording_list += [f'repr_ord{i}.activation' for i in range(4)]
        recording_list += ['nfcos']
    ...
    def plot(self):
        ...
        for t in range(len(self.recordings)):
            ...
            if t < 150:
                axes[0][1].scatter(range(4), np.clip(scalar_to_vec(12), vmin_acti, vmax_acti), color='tab:blue')
            else:
                axes[0][1].scatter(range(4), np.clip(scalar_to_vec(30), vmin_acti, vmax_acti), color='tab:orange')
            ...
            axes[0][2].scatter(range(2), np.clip(scalar_to_vec(18)[:2], vmin_acti, vmax_acti), color='tab:blue')     
            axes[0][2].scatter(range(2,4), np.clip(scalar_to_vec(18)[2:], vmin_acti, vmax_acti), color='tab:orange')     
            ...
            # axes[1][-1].imshow(self.recordings[t][0][-1], vmin=vmin_acti, vmax=vmax_acti, cmap=cmap) ## hand_cos field

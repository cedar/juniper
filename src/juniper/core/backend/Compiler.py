...
def _compile_element(element : Element, input_slots : dict[str, list[Slot]]) -> bool:
    """Compile element as either a nested circuit or a step."""
    if isinstance(element, Circuit):
        return _compile_circuit(element, input_slots)
    if isinstance(element, Step):
        return _compile_step(element, input_slots)
    raise NotImplementedError(f"No compile behavior specified for element ({element.get_local_circuit_id()})")

def _compile_circuit(circuit : Circuit, input_slots : dict[str, list[Slot]]) -> bool:
    ...
    compiled_element_map : dict = {}
    compiled_element_map[circuit] = {}

    while sub_state_updated:
        input_slots_updated, _ = _compile_input_slots(circuit, input_slots=input_slots)
        ...
        for element in circuit.element_map.values():
            ...
            element_state_updated = _compile_element(element, element_input_slots)
            ...
            if element.is_compiled and element_ref.path not in compiled_element_map[circuit]:
                compiled_element_map[circuit][element_ref.path] = element

        output_slot_updated = _compile_circuit_output_slots(circuit)
        if output_slot_updated:
            state_updated = True

    _check_circuit_compiled(circuit)
    return state_updated

def _compile_step(step : Step, input_slots : dict[str, list[Slot]]) -> bool:
    """Infer step's input slots, output slots, and buffers."""
    input_compile_info_updated, input_specs = _compile_input_slots(step, input_slots=input_slots)
    output_compile_info_updated = _compile_step_output_slots(step, input_specs=input_specs)
    buffer_compile_info_updated = _compile_buffers(step)

    state_updated = input_compile_info_updated | output_compile_info_updated | buffer_compile_info_updated
    _check_step_compiled(step)
    return state_updated
...
def _compile_circuit_output_slots(circuit : Circuit):
    ...
    for slot in circuit.output_slot_map.values():
        ...
        shape, dtype = _merge_incoming_slot_specs(circuit, sources)
...
def _check_buffer_compiled(buffer : Buffer):
...
def _check_slot_compiled(slot : Slot):
...
def _check_step_compiled(step : Step):
...
def _check_circuit_compiled(circuit : Circuit):
...
def _merge_incoming_slot_specs(element : Element, sources : list[Slot]):
...
############### Collects runtime meta data and compute kernels of compiled circuits ###############################
def _collect_compile_info(circuit : Circuit) -> CompileInfo:
    ...
    for element in circuit.element_map.values():
        ...
        if isinstance(element, Circuit):
            child_info = _collect_compile_info(element)
